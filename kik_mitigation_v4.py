...
class Kik_pg(Sched_manipulation):
    ...
    def kkik_set(self, n):
        ...
        input_cicuit = self.circ
        def q_barrier():
            dag_ = circuit_to_dag(input_cicuit)
            ...
            n_ = back.configuration().n_qubits
        ...
        k = self.forward()
        ki = self.backward()
        kik = k.compose(q_barrier()).compose(ki)        
        kik2 = k.compose(q_barrier()).compose(ki).compose(q_barrier()).compose(k).compose(q_barrier()).compose(ki)        
        final_circ = k.compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())     
        circ_list = []
        circ_list.extend([kik.compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())])
        circ_list.extend([kik2.compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())])
        circ_list.extend([final_circ])
        final_circ = q_barrier().compose(final_circ)     
        for _ in range(n):
            final_circ = kik.compose(final_circ)     
            circ_list.extend([final_circ])
        return circ_list   
...
class Kik_rc_pg(Sched_manipulation):
    def __init__(self, circ, backend):
        super().__init__(circ, backend)
    ...
    def forward(self):
        ...
        Ncr = self.circ.to_instruction().num_clbits    
        input_cicuit = self.circ
        input_cicuit = PauliTwirl()(input_cicuit)
        input_cicuit = transpile(input_cicuit, optimization_level=1, backend=self.backend)
        def gate(i):                
            circ_local = QuantumCircuit(N, Ncr)
            #
            def_i = input_cicuit.to_instruction().definition[i]
    ...
    def kkik(self, n):
        ...
        def kik_rc():
            return k_rc().compose(local_barrier_circuit()).compose(ki_rc())        
        kik = kik_rc()     
        for _ in range(n-1):
            kik = kik.compose(local_barrier_circuit()).compose(k_rc()).compose(local_barrier_circuit()).compose(ki_rc()).compose(local_barrier_circuit())    
        if n==0:
            final_circuit = k_rc().compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())     
        elif n>0:
            kkik = kik.compose(local_barrier_circuit()).compose(k_rc())
            final_circuit = kkik.compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())
        return final_circuit    
    ...
    def kkik_set(self, n):
        ...
        def ki_rc():
            return self.backward()
        def kik_rc():
            return k_rc().compose(q_barrier()).compose(ki_rc())        
        final_circ = k_rc().compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())     
        circ_list = []
        circ_list.extend([kik_rc().compose(Sched_manipulation(self.circ_with_meas, self.backend).meas_circ())])
        circ_list.extend([final_circ])
        final_circ = q_barrier().compose(final_circ)     
        for _ in range(n):
            final_circ = kik_rc().compose(final_circ)     
            circ_list.extend([final_circ])
        return circ_list   
